...
### 2026-08-30 — Scale-invariant ring score: chose density, and it cost us

- **Chose:** `density = k / n_p`, the share of a pincode's accounts that are device linked, replacing `k^2 / n_p`. Selected on train in two stages: F1 first (0.8095, tied across 14 configurations), then **train-side transfer**, applying the full-train threshold to sub-windows of train (0.6835, best).
- **Why the second stage was needed:** `raw`, `bg`, `q95` and `rank` are monotone transforms of one another inside a single window, so they give identical rankings and identical train F1. F1 cannot choose between them and an arbitrary tie-break was deciding. Transfer is the property being bought, so transfer is what gets measured, on train.
- **Why density and not the ratio-to-chance forms:** the analytic lift, `k / (n(n-1)p_pair)`, drifted worse than the original and in the opposite direction, 2.0e4 down to 3.4e3 across window lengths, because it over-rewards small pincodes. The empirical background forms, dividing by the window's median or 95th-percentile cluster, were tied on train F1 but transferred badly on train sub-windows (0.15 to 0.37 against density's 0.68).
- **What it cost:** a lot. Test PR AUC fell from **0.5811 to 0.2047** at the same window, because `k^2` rewards larger components and `k/n_p` does not, so a three-person household on a six-account pincode now ties a real ring. **A worse number that transfers would have been the right trade. This is a worse number that only partly transfers.**
- **Drift is reduced, not removed:** across test windows of 10/20/30/50%, raw moves 1.125 to 2.286 (2.03x) and density 0.375 to 0.571 (1.52x). About a quarter of the drift is gone.
- **The frozen cut still does not fire on the 30% test split, and the reason is not scale.** Per ring, `n_p` is stable across window lengths (r00 7/8/8, r02 10/10/11) and so is `k`. What breaks the threshold is that **r01, at density 0.5714, is the ring that sets the train cut at 0.5, and it is absent from the test window entirely** because it was caught before the split. The two rings present sit at 0.25 and 0.40. With three rings, between-ring variation dominates window length.
- **Disclosed:** `rank`, the quantile-position form, does fire on test with precision 1.0000 and recall 0.5556. It was **not** selected, because its train F1 is 0.5714 against the 0.8095 tie, so it is not competitive on the split we are allowed to choose on. It looks good on test and picking it for that reason would be selection on test. Recorded here so the choice is visible rather than buried.
- **What this means for a deployable version:** the units are now right, and the remaining problem is calibration against a population of three rings. That is a sample-size problem, not a normalisation one, and no further reshaping of the score will fix it.
