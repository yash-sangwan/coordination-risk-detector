...
- **How we fixed it:** Row-weight the benign probability as `exp(-lam)`, and compute attack uniqueness per field by simulating each declared generator rather than assuming it. Four fields shared the broken predictor and all four were fixed. Corrected deltas: `contact` -0.023, `card.last4` -0.011, `vpa` -0.008, `mc.device_id` -0.000. `email` still exceeds by +0.058 against a predictor spread of 0.006, so that one is a real flag and stays open.
